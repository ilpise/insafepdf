...
$(document).ready(function(){
  ...
  pagina = $('input#matricola').val().toUpperCase()+".svg";
  cartella =$('input#matricola').val().toLowerCase();

  $('.my-svg-container').attr('style', 'width: 100%; height: 960px;');
  $('.my-svg-container').children('.my-svg-container').addClass('new-svg-container').removeClass('my-svg-container');
  $('.new-svg-container').attr('style', 'width: 100%; height: 100%;');


//  $('.my-svg-container').children('.my-svg-container').removeClass('my-svg-container');


  //RETRIEVE FILELIST
  $.ajax({
        type: 'POST',
        url: 'common/listing.php',
        // data: 'macchina='+mainmachine,
        data: 'macchina='+cartella,
        dataType: 'json',
        async: false,
        cache: false,
        timeout: 30000,
  ...
  $("#mytreeview").on("ready.jstree", function(e, data) {
    $('#mytreeview').jstree('select_node', '1');
    $('#mytreeview').jstree("open_node", '1')
    var alto =  $( window ).height();
    alto = alto - (alto*0.18);
    //alert(alto);
    var stilealb = 'padding: 20px 20px; overflow: auto; height:'+alto+'px;';
    $('#basicTree').attr('style',stilealb);
  });
  ...
  var mainmachine = pathpart[4]
  var pagina = window.location.pathname.split('/')[6].toLowerCase();
  var cartella = window.location.pathname.split('/')[4].toLowerCase()+".svg";*/
  if(currentav) {
    var tavtoload = currentav+'.svg';
  } else {
    var tavtoload = pagina;
  }
  Snap.load('../xcompany/az_insafe/macchine/'+cartella+'/svg/'+tavtoload, onSVGLoaded ) ;
});
